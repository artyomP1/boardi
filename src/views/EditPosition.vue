...
export default {
  ...
  async created() {
    ...
    try {
      ...
      const position = await this.getPosById(this.id);
      this.formScheme = position.schema.map((item) => {
        item.value = position.Payload[item.inputName];
        return item;
      });
    } catch (e) {
  ...
  methods: {
    ...
    async editPos() {
      ...
      try {
        ...
        const res = await this.updatePos({ id: this.id, data });
        this.$message({
